...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var player3DropDown = new FlxUIDropDownMenu(290, 100, FlxUIDropDownMenu.makeStrIdLabelArray(characters, true), function(character:String)
			{
				_song.gfver = characters[Std.parseInt(character)];
				updateHeads();
			});
			player3DropDown.selectedLabel = _song.gfver;
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.keys.pressed.ALT)
			mouse = FlxG.mouse.pressed;
		else if (!FlxG.keys.pressed.ALT)
			mouse = FlxG.mouse.justPressed;
	...
	{
		trace(Paths.json(song.toLowerCase()+"/"+song.toLowerCase()));
		if (FileSystem.exists(Paths.json(song.toLowerCase()+"/"+song.toLowerCase()))){
			PlayState.SONG = Song.loadFromJson(song.toLowerCase(), song.toLowerCase());
			LoadingState.loadAndSwitchState(new states.editors.ChartingState());
		}
		else engine.ErrorCheck.loadSongError(Paths.json(song.toLowerCase()+"/"+song.toLowerCase()));
	}
